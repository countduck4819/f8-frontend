...
const actionToVoice = function (list, stringVoice, link) {
    ...
    if (index !== -1 && check === 1) {
        ...
        var getString = stringVoice.slice(
            stringVoice.indexOf(wordInList) + wordInList.length + 1
        );
        setTimeout(() => {
            window.open(
                `${link}${getString
...
btnVoice.addEventListener("click", () => {
    ...
    recognition.onresult = (e) => {
        ...
        var openName = Object.keys(brain).find((name) =>
            stringVoice.includes(name)
        );
        if (openName && check === 1) {
            check = 0;
            setTimeout(() => {
        ...
        actionToVoice(
            music,
            stringVoice,
            "https://zingmp3.vn/tim-kiem/tat-ca?q="
        );
        actionToVoice(
            video,
            stringVoice,
